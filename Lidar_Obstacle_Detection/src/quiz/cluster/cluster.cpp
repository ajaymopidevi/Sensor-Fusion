...
void clusterHelper(int i, const std::vector<std::vector<float>>& points, std::vector<int>& cluster, std::vector<bool>& processed, KdTree* tree, float distanceTol)
{
	...
	//std::cout<<"{"<<points[i][0]<<","<<points[i][1]<<"}   "<<nearby.size()<<endl;
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	while(i < points.size())
	{
		...
		//std::cout<<"Cluster:"<<cluster.size()<<endl;
...
int main ()
{
	...
	std::vector<std::pair<std::vector<float>,int>> checkPoints;
	for(int i=0;i<points.size();i++)
	{
		checkPoints.push_back(std::pair<std::vector<float>,int>(points[i],i));
	}
	tree->insertCheck( checkPoints.begin(), checkPoints.end(), 0);
    ...
    //	tree->insert(points[i],i); 
  	...
  	std::vector<int> nearby = tree->search({0.2,-7.1},3.0);
	std::cout<<"Size:"<<nearby.size()<<endl;
