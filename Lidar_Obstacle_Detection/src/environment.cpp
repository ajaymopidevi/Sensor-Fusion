...
{
    //Downsample and crop the pcd points between min and max
    ...
    bool pcd_libraries = true;
    if (pcd_libraries)
    {
        //PointCloud Segmentation using PCL libraies
        segmentCloud = pointProcessor.SegmentPlane(inputCloud, 100, 0.4);

        //Clutering using PCL libaries
        cloudClusters = pointProcessor.Clustering(segmentCloud.first, 0.53, 10, 500);
    }
    else
    {
        //PointCloud Segmentation
        segmentCloud = pointProcessor.SegmentPlaneRansac3D(inputCloud, 100, 0.4);

        //Clutering
        cloudClusters = pointProcessor.euclideanCluster(segmentCloud.first, 0.53, 10, 500);
    }
    ...
    //Add bounding boxes for each cluster
